...
namespace Mr.Hwang
{
    public class FSM_PlayerState_Skill : VMyState<FSM_PlayerState>
    {
        ...
        {
            GameObject spawnWeapon = ObjectPoolManager.Instance.SpawnFromPool(_player.currentBW.projectileType,
                transform.position, Quaternion.identity);
            BasicWeapon bw = spawnWeapon.GetComponent<BasicWeapon>();
            Vector3 mousePos = Input.mousePosition;
            Vector3 mouseWorldPos = Camera.main.ScreenToWorldPoint(new Vector3(mousePos.x, mousePos.y,
